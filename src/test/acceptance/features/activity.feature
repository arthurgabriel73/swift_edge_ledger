Feature: Activity
  ...
  Scenario: Authorize a credit card activity
    ...
    When I send an activity request with the user account "987677421" authorizes a credit card activity with amount in cents "10000" for merchant "Supermarket XYZ" with mcc code "4213"
    Then I should receive a response with status code 201
    And the response should contain a field code with the value "00"
    And the system should record the activity with amount in cents "10000" and category "FOOD"
    And the account balance amount in cents should be updated to "90000" for the user account "987677421" and category "FOOD"
